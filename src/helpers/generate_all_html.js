...
function generate_all_html(language) {
    ...
    try {
        ...
        mjmlFiles.forEach(fileName => {
            ...
            //-- Read JSON footer translation file --//
            const jsonFooterFilePath = path.join(langPath, language, `footer.json`)
            ...
            const jsonFooterData = fs.readFileSync(jsonFooterFilePath, "utf8")
            const translation = JSON.parse(jsonData)
            const translationFooter = JSON.parse(jsonFooterData)
            const transFull = { ...translation, ...translationFooter }

            //-- Replace placeholders in MJML template with translation data --//
            let mergedTemplate = mjmlTemplate
            Object.keys(transFull).forEach(key => {
                const regex = new RegExp(`{{${key}}}`, "g")
                mergedTemplate = mergedTemplate.replace(regex, transFull[key])
            })
